...
void LLAgentCamera::updateLookAt(const S32 mouse_x, const S32 mouse_y)
{
	...
	else
	{
		...
		else if (cameraThirdPerson())
		{
			...
			// range from -.5 to .5
			F32 x_from_center = 
				    ((F32) mouse_x / (F32)(gHMD.isHMDMode() ? gHMD.getUIWidth() : gViewerWindow->getWorldViewWidthScaled()) ) - 0.5f;
			F32 y_from_center = 
				    ((F32) mouse_y / (F32)(gHMD.isHMDMode() ? gHMD.getUIHeight() : gViewerWindow->getWorldViewHeightScaled()) ) - 0.5f;
