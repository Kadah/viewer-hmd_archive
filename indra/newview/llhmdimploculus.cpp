...
BOOL LLHMDImplOculus::preInit()
{
    ...
    gHMD.isPreDetectionInitialized(ovr_Initialize());
    if (!gHMD.isPreDetectionInitialized())
    {
        return FALSE;
    }
...
void LLHMDImplOculus::onIdle()
{
    ...
    if (mHMD)
    {
        ...
        bool wasHMDSensorConnected = gHMD.isHMDConnected();
        //bool wasHMDDisplayEnabled = gHMD.isHMDDisplayEnabled();
        gHMD.isHMDConnected(mHMD && (mHMD->HmdCaps & ovrHmdCap_Present) != 0);
        gHMD.isHMDSensorConnected(mHMD && (mHMD->HmdCaps & (ovrHmdCap_Available | ovrHmdCap_Captured)) != 0);
        gHMD.isHMDDisplayEnabled(mHMD && mHMD->ProductName && mHMD->ProductName[0] != 0);
    ...
    mHMDFrameTiming = ovrHmd_BeginFrame(mHMD, 0);
    ovrTrackingState ss = ovrHmd_GetTrackingState(mHMD, mHMDFrameTiming.ScanoutMidpointSeconds);

    //gHMD.isVisionTrackingEnabled((ss.StatusFlags & ovrStatus_PositionTracked) != 0);
